...
from conformer.nets_utils import get_activation
...
from transformer.multi_layer_conv import (
    Conv1dLinear,
    MultiLayeredConv1d,
)
...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.linear_output = torch.nn.Linear(attention_dim, 1)
    ...
    def forward(self, xs, masks):
        ...
        xs = self.linear_output(xs)
        return xs, masks


if __name__ == "__main__":
    x = torch.randn(100, 640, 64).cuda()
    encoder = Encoder(
        idim=64,
        attention_dim=64,
        attention_heads=4,

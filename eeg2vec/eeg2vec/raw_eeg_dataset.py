...
class EegDataset(FairseqDataset):
    ...
    def crop_to_max_size_eeg(self, wav, target_size, start=None):
        size = len(wav)
        diff = size - target_size
        if diff <= 0:
            return wav, 0
    ...
    def collater_audio(self, audios, audio_size, audio_starts=None):
        ...
        # print("audio_feat_shape", audio_feat_shape)
        ...
        for i, audio in enumerate(audios):
            ...
            if diff == 0:
                ...
                collated_audios[i], audio_starts[i] = self.crop_to_max_size_eeg(
    ...
    def collater(self, samples):
        ...
        #         padding_mask[i, diff:] = True
        #     else:
        #         collated_sources[i], _ = self.crop_to_max_size_eeg(source, target_size)
        collated_sources, padding_mask, _ = self.collater_audio(sources, target_size)
        # print("collated_sources", collated_sources.size())
...
class FileEegDataset(EegDataset):
    def __init__(
        self,
        manifest_path,
        sample_rate,
    ...
    def __getitem__(self, index):
        ...
        # print("path_or_fp", path_or_fp)
        eeg_data =  np.load(path_or_fp).astype(np.float32)
        feats = torch.from_numpy(eeg_data).float()

...
if __name__ == "__main__":
    ...
    results_folder = os.path.join(experiments_folder, "results")
    ...
    if only_evaluate:
        ...
        val_files = [x for x in glob.glob(os.path.join(data_folder, "test_-_*")) if os.path.basename(x).split("_-_")[-1].split(".")[0] in features]
        ...
        model.fit(
            ...
            callbacks=[
                ...
                tf.keras.callbacks.ReduceLROnPlateau(monitor='val_accuracy', factor=0.1, patience=15, min_lr=0.0001)
            ],
        )
